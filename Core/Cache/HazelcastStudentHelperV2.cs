...
namespace Cache
{
    ...
    internal sealed class HazelcastStudentHelperV2 : IHazelcastStudentHelperV2
    {
        ...
        private const string _mapMark = "MergedMarksV2";
...
SELECT 
    mm.StudentName AS StudentName
    ,ROUND(AVG(mm.MarkValue), 1) AS AverageMark
    ,COUNT(DISTINCT mm.ExamName) AS ExamCount
FROM {_mapMark} mm
...
SELECT 
    ...
    ,ROUND(m.TotalMarks, 1) AS TotalMarks
FROM (
  SELECT 
    StudentName
    ,StudentRollNumber
...
{_mapMark} (
  ...
  'valueFormat'='json-flat'
                ...
                var map = await client.GetMapAsync<long, HazelcastJsonValue>(_mapMark).ConfigureAwait(false);
